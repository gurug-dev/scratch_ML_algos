...
def load_docs(docs_dirname:str) -> Sequence[Sequence]:
    ...
    docs = []
    # YOUR CODE HERE
    files = [file for file in filelist(docs_dirname) if '.txt' in file]
    for doc in files:
        word_vec = words(get_text(doc))
        if len(word_vec) > 0: docs.append(word_vec)
    return docs
...
def vocab(neg:Sequence[Sequence], pos:Sequence[Sequence]) -> dict:
    ...
    # YOUR CODE HERE
    V['__unknown__'] = 0
    flatten_neg = [word for word_list in neg for word in word_list]
    flatten_pos = [word for word_list in pos for word in word_list]
    combined_list = set().union(flatten_neg, flatten_pos)
    for i, word in enumerate(sorted(combined_list)):
        V[word] = i + 1   #index + 1 as 0 is __unknown__ 
    return V
...
def vectorize_docs(docs:Sequence, V:dict) -> np.ndarray:
    ...
    """
    # YOUR CODE HERE
    return np.vstack([vectorize(V, word_list) for word_list in docs])
...
class NaiveBayes621:
    ...
    def fit(self, X:np.ndarray, y:np.ndarray) -> None:
        ...
        """
        # YOUR CODE HERE
        self.prob_1 = np.sum(y) / len(y)
        self.prob_0 = 1 - self.prob_1
        self.prob_w_given_1 =  (X[np.equal(y,1)].sum(axis=0) + 1) / (np.sum(X[np.equal(y,1)]) + X.shape[1])
        self.prob_w_given_0 = (X[np.equal(y,0)].sum(axis=0) + 1) / (np.sum(X[np.equal(y,0)]) + X.shape[1])
    ...
    def predict(self, X:np.ndarray) -> np.ndarray:
        ...
        """
        # YOUR CODE HERE
        class_1 = np.log(self.prob_1) + np.multiply(X,np.log(self.prob_w_given_1)).sum(axis = 1)
        class_0 = np.log(self.prob_0) + np.multiply(X,np.log(self.prob_w_given_0)).sum(axis = 1)
        return np.where(class_1 < class_0, 0, 1)
...
def kfold_CV(model, X:np.ndarray, y:np.ndarray, k=4) -> np.ndarray:
    ...
    for train_index, test_index in kf.split(X):
        ...
        accuracies.append(np.sum(y_test==y_pred)/ len(y_test))        
